##############################################################
##    EXTRACT NOAA nClimGrid DATA FOR GRCA   ###############
##############################################################
...
# Initials
data.dir <- "D:/NOAA-nClimGrid" #location for text files
...
nps_boundary <- st_read('C:\\Users\\gknowlton\\OneDrive - DOI\\Documents\\GIS\\spatial-data\\nps_boundary\\nps_boundary.shp')
...
pnt.list<-list.files(path=data.dir, pattern="conus") #list all files by var
...
dfx<-spTransform(dfx,CRSobj = "+init=epsg:5070") #reproj sp obj
...
projection(x) = CRS("+init=epsg:5070")

# Plot park over raster
Sp_park<-spTransform(Sp_park,CRSobj = "+init=epsg:5070") #reproj sp obj
