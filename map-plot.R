...
topo <- stack('C:/Users/achildress/DOI/NPS-WRST-Resource Stewardship Strategy - Climate/3.0 Climate futures development (Summer 2021)/Data/Boundary_shapefiles/HYP_HR_SR_W/HYP_HR_SR_W.tif') # read in as stack so can see RBG layers
...
ggplot() +
  geom_raster(data = t_df ,aes(x = x, y = y,alpha=HYP_HR_SR_W.1), show.legend=FALSE) + #it's a multiband raster so alpha is band I wanted to use
  geom_stars(data = nc_crop, alpha = 0.4) + # the WB layer I was plotting - alpha is transparency
