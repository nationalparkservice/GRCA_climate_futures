...
pnt.list<-list.files(path=data.dir, pattern=".tave.conus") #list all files by var
#tmax, tmin, tave, prcp
...
nps_boundary <- st_read('C:/Users/gknowlton/OneDrive - DOI/Documents/GIS/nps_boundary/nps_boundary.shp') # Directory Annie comp
...
yr<-seq(1925,2020,1)
Sp_park<-spTransform(Sp_park,CRSobj = "+init=epsg:5070") # project to Alaska Albers
...
# Write csv's
prcp_grca_df <- data.frame(m_pr = cellStats(m_pr, stat = 'mean'), year = yr)

row.names(prcp_grca_df)<-NULL

m_temp_park <-data.frame(m_temp_park=cellStats(m_temp_park,stat='mean'),year=yr)

row.names(m_temp_park)<-NULL

write.csv(m_temp_park,"./output/csvs/tave_grca.csv",row.names=F)

write.csv(prcp_grca_df, "./output/csvs/prcp_grca.csv", row.names = FALSE)

# ---  PLOTS ----------------------------------------- #      

# read in csvs
prcp <- read.csv("./output/csvs/prcp_grca.csv")
tmax <- read.csv("./output/csvs/tmax_grca.csv")
tmin <- read.csv("./output/csvs/tmin_grca.csv")
tave <- read.csv("./output/csvs/tave_grca.csv")
...
tave$tave <- tave$m_temp_park
tmin$tmin <- tmin$m_temp_park
tmax$tmax <- tmax$m_temp_park
prcp$prcp <- prcp$m_pr
