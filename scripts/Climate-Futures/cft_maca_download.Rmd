...
#   filter(`Model abbreviation`== c("MIROC-ESM-CHEM")) %>% # MRI-CGCM3
#   filter(Scenario == c("RCP 4.5", "RCP 8.5")) %>% 
#   pull("Available variable")
input_variables<-inputs$variable_names %>% 
  filter(`Variable abbreviation` %in% c("pr","tasmax","tasmin")) %>% 
  filter(`Model abbreviation` %in% c("MIROC-ESM-CHEM", "MRI-CGCM3")) %>% 
...
``` {r}
site = "GRCA"
centroids <- st_read("C:\\Users\\gknowlton\\OneDrive - DOI\\Documents\\GIS\\nps_boundary_centroids\\nps_boundary_centroids.shp")
GGCL <- st_read("C:\\Users\\gknowlton\\OneDrive - DOI\\Documents\\GRCA\\GRCA_maps\\GIS\\GGCL\\GGCL.shp")
GGCL <- st_transform(GGCL, st_crs(centroids))
bb <- st_bbox(GGCL)
