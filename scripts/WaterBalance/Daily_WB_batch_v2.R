...
ggplot(Annual, aes(x=deficit, y=AET, colour=CF)) + geom_point(size=3)+ geom_smooth(method="lm", se=FALSE, size=2)+
  ...
  labs(
    ...
    colour = "GCM",
    title = paste("Water Balance for CARE",sep="")  
...
ggsave(paste("Water Balance-CARE",".png",sep=""), path = OutDir, width = 15, height = 9)
...
ggplot(Annual, aes(x=deficit, colour=CF,fill=CF,linetype=CF),show.legend=F) +geom_density(alpha=0.3,size=1.5) +
  ...
  labs(y = "Density",
       x = "Annual moisture deficit (in)",
       title = paste("CARE Water Deficit for GCMs (2025-2055) and Historical Period (1950-1999)",sep=" ")) +
...
ggsave(paste("CARE-Deficit_density_panel.png",sep=""), path = OutDir, width = 15, height = 9)
...
ggplot(Annual, aes(x=SOIL_in, colour=CF,fill=CF,linetype=CF),show.legend=F) +geom_density(alpha=0.3,size=1.5) +
  ...
  labs(y = "Density",
       ...
       title = paste("CARE Soil Moisture for GCMs (2025-2055) and Historical Period (1950-1999)",sep=" ")) +
  theme(axis.text = element_text(size=20), axis.title = element_text(size=20), legend.text=element_text(size=14),
        plot.title=element_text(size=22, hjust=0),legend.position = c(.8,.8)) 

ggsave(paste("CARE-SOIL_in_density_panel.png",sep=""), path = OutDir, width = 15, height = 9)
...
plot_1 <- ggplot() +
  ...
  scale_fill_manual(name   = "Veg biomes",
                    ...
                    labels = names(color),
                    values = color) + 
  labs(title=paste("CARE water balance effects on biome",sep=""),
...
plot_1 + geom_point(data=Annual, aes(x=sum_d, y=sum_aet, colour=CF), size=3) + 
  ...
  scale_colour_manual("Scenario",values=colors3)
ggsave(paste("CARE-WB-biome effects.png",sep=""), path = OutDir, width = 15, height = 9)
